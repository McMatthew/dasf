...
import type {
    ...
    DasfJsonPayloadItems, DasfInterface
} from './types';
...
const resolveInterface = (prop: DasfJsonPayloadProperty, doc: DasfDocument): DasfJsonPayloadProperty => {
    if (typeof prop.type === 'string' && prop.type.startsWith('$interfaces.')) {
        const interfacePath = prop.type.split('.').slice(1);
        let resolved: DasfInterface | DasfJsonPayloadProperty | undefined = doc.$interfaces;
        for (const key of interfacePath) {
            if (!resolved || typeof resolved !== 'object' || !(key in resolved)) {
                ...
                return { type: 'unresolved', description: `Error: Could not resolve ${prop.type}` };
            }
...
const JsonPayloadView: React.FC<{
    ...
    const renderProperties = (props: { [key: string]: DasfJsonPayloadProperty | any } | DasfJsonPayloadItems) => {
        // This condition distinguishes the 'items' part of an array from a regular object property map.
        if ('type' in props && typeof props.type === 'string' && !Object.keys(props).some(k => ['description', 'takeoffPort'].includes(k))) {
            const itemProp = props as DasfJsonPayloadProperty;

            if (itemProp.type.startsWith('$interfaces.') && resolutionStack.includes(itemProp.type)) {
                return <div style={{ paddingLeft: `${level * 20}px` }}>Items: <code>{itemProp.type}</code> <em style={{ color: 'red' }}>(Circular Reference)</em></div>;
            }

            const resolvedItems = resolveInterface(itemProp, doc);
            const newStack = itemProp.type.startsWith('$interfaces.') ? [...resolutionStack, itemProp.type] : resolutionStack;
            // @ts-ignore
            ...
            return (
                <div style={{ paddingLeft: `${level * 20}px` }}>
                    ...
                    {subProps && <JsonPayloadView data={subProps} doc={doc} level={level + 1} resolutionStack={newStack} />}
                    {resolvedItems.items && <JsonPayloadView data={resolvedItems.items} doc={doc} level={level + 1} resolutionStack={newStack} />}
                </div>
            );
        }
        ...
        return (
            <ul style={{ paddingLeft: `${level * 20}px`, listStyle: 'none' }}>
                {Object.entries(propertiesToRender).map(([key, prop]) => {
                    if (typeof prop.type === 'string' && prop.type.startsWith('$interfaces.') && resolutionStack.includes(prop.type)) {
                        return (
                            <li key={key}>
                    ...
                    const newStack = (typeof prop.type === 'string' && prop.type.startsWith('$interfaces.')) ? [...resolutionStack, prop.type] : resolutionStack;

                    return (
                        <li key={key}>
                            <code>{key}</code>: <strong>{resolvedProp.type}</strong>
                            {resolvedProp.description && <em> - {resolvedProp.description}</em>}
                            {subProps && <JsonPayloadView data={subProps} doc={doc} level={level + 1} resolutionStack={newStack} />}
                            {resolvedProp.items && <JsonPayloadView data={resolvedProp.items} doc={doc} level={level + 1} resolutionStack={newStack} />}
                        </li>
                    );
                })}
            </ul>
...
const PayloadView: React.FC<{ title: string, payload: DasfPayload, doc: DasfDocument }> = ({ title, payload, doc }) => (
    <div className="endpoint-section">
        <h4>{title}</h4>
        ...
        {payload.body && (
            <>
                ...
                {payload.type === 'json' && <JsonPayloadView data={payload.body as { [key: string]: DasfJsonPayloadProperty }} doc={doc} />}
...
const EndpointView: React.FC<{ path: string, endpoint: DasfEndpoint, doc: DasfDocument }> = ({ path, endpoint, doc }) => {
    return (
        <article className="endpoint-view">
            ...
            {endpoint.sentPayload && <PayloadView title="Sent Payload" payload={endpoint.sentPayload} doc={doc} />}
            {endpoint.receivedPayload && <PayloadView title="Received Payload" payload={endpoint.receivedPayload} doc={doc} />}
        </article>
    );
};

const DasfViewer: React.FC<{ doc: DasfDocument, title: string }> = ({ doc, title }) => {
    const endpoints = Object.entries(doc).filter(([key]) => key !== '$interfaces');
